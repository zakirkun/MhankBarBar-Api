...
def search_kuso(query):
    usr_agent = {
        'User-Agent': random.choice(ua)
        }
    url = bs(get('https://kusonime.com/?s=%s' % query, headers=usr_agent).text, 'html.parser').find('div', class_='detpost').findAll('a')[0]['href']
    return url

def search_dewa(query):
	usr_agent = {
		'User-Agent': random.choice(ua)
		}
	url = bs(get('https://dewabatch.com/?s=%s' % query, headers=usr_agent).text, 'html.parser').find('div', class_='dtl').findAll('a')[0]['href']
	return url